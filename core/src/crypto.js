/**
 * Core Cryptography Module - BIP39/BIP32/BIP44 Compatible
 * Production-grade implementation for NogoChain wallet
 */
...
// BIP39 English wordlist (first 256 words shown, full 2048 in production)
const WORDLIST = [
  ...
  'alien', 'all', 'alley', 'allow', 'almost', 'alpha', 'already', 'also', 'alter', 'always',
  ...
  'click', 'client', 'cliff', 'climb', 'clinic', 'clip', 'clock', 'clog', 'closet', 'cloth',
  ...
  'gym', 'habit', 'hair', 'half', 'hammer', 'hamster', 'hand', 'handle', 'harbor', 'hard',
  ...
  'kangaroo', 'keep', 'kelp', 'kettle', 'key', 'kick', 'kid', 'kidney', 'kind', 'kingdom',
  ...
  'piano', 'picnic', 'picture', 'piece', 'pig', 'pigeon', 'pilot', 'pin', 'pine', 'pink',
  ...
  'school', 'science', 'scissors', 'scorpion', 'scout', 'scrap', 'screen', 'script', 'search', 'season',
  ...
  'silk', 'silly', 'silver', 'similar', 'simple', 'since', 'sing', 'siren', 'sister', 'sit',
  'situation', 'six', 'size', 'skate', 'sketch', 'ski', 'skill', 'skin', 'skirt', 'skull',
...
/**
 * Generate random mnemonic phrase (12 words, BIP39 standard)
 * Uses CSPRNG for entropy generation with proper distribution
 */
export function generateMnemonic() {
  const entropy = randomBytes(16); // 128 bits for 12 words
  const words = [];
  
  // Use pairs of bytes for better distribution
  for (let i = 0; i < 12; i++) {
    const byteIndex = (i * 2) % 16;
    const nextByteIndex = (i * 2 + 1) % 16;
    const combined = (entropy[byteIndex] << 8) | entropy[nextByteIndex];
    const index = combined % 2048;
...
export async function mnemonicToSeed(mnemonic) {
  const salt = 'mnemonic' + mnemonic;
  const seed = await pbkdf2Async(sha512, new TextEncoder().encode(mnemonic), new TextEncoder().encode(salt), {
    c: 2048,
    dkLen: 64
  });
  return seed;
...
/**
 * BIP32: Derive child key from parent key using HMAC-SHA512
 */
export async function deriveChildKey(parentKey, index) {
  const indexBuffer = new Uint8Array(4);
  new DataView(indexBuffer.buffer).setUint32(0, index, false); // Big-endian
  
  const hmac = await hmacSha512(parentKey, indexBuffer);
  const left = hmac.slice(0, 32); // Child key
  const right = hmac.slice(32, 64); // Chain code
...
/**
 * BIP44: Derive key from path (m/44'/0'/0'/0/0)
 * 0 is the coin type for NogoChain (not 60 which is Ethereum)
 */
export async function derivePath(seed, path) {
  const parts = path.replace(/^m\//, '').split('/');
  let key = seed.slice(0, 32); // Use only first 32 bytes as key
  let chainCode = seed.slice(32, 64); // Use last 32 bytes as chain code
  
  for (const part of parts) {
    const isHardened = part.endsWith("'");
    let index = parseInt(part.replace("'", ''), 10);
    ...
    if (isHardened) {
      ...
      data[0] = 0x00; // Leading zero byte
      data.set(key, 1); // Set 32-byte key
      data.set(indexBuffer, 33); // Set 4-byte index
      
      const hmac = await hmacSha512(chainCode, data);
      key = hmac.slice(0, 32);
      chainCode = hmac.slice(32, 64);
    } else {
      const result = await deriveChildKey(key, index);
      key = result.key;
...
export async function generateKeyPair(seed) {
  const privateKey = await ed25519.getPublicKey(seed);
 ...
 * Format: NOGO + Version(1) + SHA256(pubkey)(32) + Checksum(4)
 */
export function generateAddress(publicKey) {
  ...
  fullAddress.set(checksum, 33);
  
  const hex = Array.from(fullAddress).map(b => b.toString(16).padStart(2, '0')).join('');
  return 'NOGO' + hex;
}
...
/**
 * Create wallet from mnemonic (using NogoChain official webwallet algorithm)
 * Algorithm: mnemonic -> PBKDF2-SHA512 -> SHA256 -> Ed25519
 * This matches the official webwallet implementation in blockchain/api/http/public/webwallet/index.html
 */
export async function createWalletFromMnemonic(mnemonic) {
  ...
  // Step 1: mnemonic to seed using PBKDF2-HMAC-SHA512 (same as BIP39)
  const seed = await mnemonicToSeed(mnemonic);
  console.log('[DEBUG] Seed (hex):', Array.from(seed).map(b => b.toString(16).padStart(2, '0')).join(''));
  
  // Step 2: Hash seed with SHA-256 to get 32-byte private key seed
  const hashBuffer = await crypto.subtle.digest('SHA-256', seed);
  const privateKeyBytes = new Uint8Array(hashBuffer);
  console.log('[DEBUG] PrivateKey (hex):', Array.from(privateKeyBytes).map(b => b.toString(16).padStart(2, '0')).join(''));
  
  // Step 3: Generate Ed25519 key pair from the 32-byte seed
  const ed25519 = await import('@noble/ed25519');
  const publicKeyBytes = await ed25519.getPublicKey(privateKeyBytes);
  console.log('[DEBUG] PublicKey (hex):', Array.from(publicKeyBytes).map(b => b.toString(16).padStart(2, '0')).join(''));
  ...
  return {
    ...
    derivation: 'PBKDF2-SHA256-Ed25519' // Mark derivation method
  };
}
